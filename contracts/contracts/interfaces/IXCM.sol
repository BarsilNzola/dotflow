// SPDX-License-Identifier: MIT
pragma solidity ^0.8.23;

interface IXCM {
    enum MessageStatus { Pending, Executed, Failed, Cancelled, Expired }

    struct XCMMessage {
        bytes32 id;
        uint32  sourceChainId;
        uint32  destinationChainId;
        address sender;
        address recipient;
        address asset;
        uint128 amount;
        uint64  timestamp;
        uint64  timeout;
        bytes32 hash;
        MessageStatus status;
    }

    struct ParachainAsset {
        bytes32 assetId;
        uint128 amount;
        bool    isNative;
    }

    // Kept for ParachainAdapter ABI compatibility — not used by new executor
    struct XCMInstruction {
        uint32  destinationChainId;
        address sender;
        address recipient;
        address asset;
        uint128 amount;
        bytes   callData;
        uint64  weight;
        uint128 transactWeight;
        uint64  timeout;
    }

    event XCMMessagePrepared(
        bytes32 indexed messageId,
        uint32  indexed destinationChainId,
        address indexed sender,
        address         recipient,
        address         asset,
        uint128         amount,
        uint64          timeout
    );
    event XCMMessageExecuted(bytes32 indexed messageId, bytes32 xcmHash, bool success);
    event XCMMessageCancelled(bytes32 indexed messageId);
    event XCMMessageExpired(bytes32 indexed messageId);
    event XCMSent(bytes32 indexed messageId, bytes destination, bytes xcmMessage);

    error ChainNotSupported(uint32 chainId);
    error InvalidRecipient();
    error InsufficientFee(uint256 required, uint256 provided);
    error MessageNotFound(bytes32 messageId);
    error MessageAlreadyExecuted(bytes32 messageId);
    error MessageExpired(bytes32 messageId);

    function sendParachainAssets(
        uint32 parachainId,
        address recipient,
        ParachainAsset[] calldata assets,
        bytes calldata callData,
        uint64 timeout
    ) external payable returns (bytes32 messageId);

    function getMessageStatus(bytes32 messageId) external view returns (MessageStatus);
    function getMessageDetails(bytes32 messageId) external view returns (XCMMessage memory);
    function calculateFee(uint32 destinationChainId, uint64 weight, uint256 amount) external view returns (uint256);
    function cancelMessage(bytes32 messageId) external returns (bool);
}
